...
int main (int argc, const char *argv[])
{
	...
	cout << "PSNR noiseImage: " << cv::PSNR(originalImage, noiseImage) << endl;
	cout << "PSNR m_image: " << cv::PSNR(originalImage, m_image) << endl;
	cout << "PSNR g_image: " << cv::PSNR(originalImage, g_image) << endl;

	cout << "msssim noiseImage: " << msssim->compute(originalImage, noiseImage) << endl;
	cout << "msssim m_image: " << msssim->compute(originalImage, m_image) << endl;
	cout << "msssim g_image: "<< msssim->compute(originalImage, g_image) << endl;
	...
	cout << "vifp noiseImage: " << vifp->compute(originalImage, noiseImage) << endl;
	cout << "vifp m_image: " << vifp->compute(originalImage, m_image) << endl;
	cout << "vifp g_image: " << vifp->compute(originalImage, g_image) << endl;

	//originalImage.convertTo(originalImage, CV_32F, 1.0 / 255);
	//noiseImage.convertTo(noiseImage, CV_32F, 1.0 / 255);
	//m_image.convertTo(m_image, CV_32F, 1.0 / 255);
	//g_image.convertTo(g_image, CV_32F, 1.0 / 255);

	//cout << "phvs noiseImage: " << phvs->compute(originalImage, noiseImage) << endl;
	//cout << "phvs m_image: " <<    phvs->compute(originalImage, m_image) << endl;
	//cout << "phvs g_image: " <<    phvs->compute(originalImage, g_image) << endl;
}
